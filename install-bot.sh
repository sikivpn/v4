...
Name=$(curl -sS https://raw.githubusercontent.com/sikivpn/permission/main/ipmini | grep $MYipmini | awk '{print $2}')
...
PERMISSION () {
    MYipmini=$(curl -sS ipminiv4.icanhazipmini.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/sikivpn/permission/main/ipmini | awk '{print $4}' | grep $MYipmini)
    if [ "$MYipmini" = "$IZIN" ]; then
    Bloman
    else
...
red "Your scripminit need to update first !"
exit 0
...
[[ ! -f "/etc/ipmini" ]] && wget -qO- ipminiv4.icanhazipmini.com > /etc/ipmini
...
ISP=`curl -sS ipmini-api.com | grep -w "isp" | awk '{print $3,$4,$5,$6,$7,$8,$9}' | cut -d'"' -f2 | cut -d',' -f1 | tee -a /etc/afak.conf`
CITY=`curl -sS ipmini-api.com | grep -w "city" | awk '{print $3}' | cut -d'"' -f2 | tee -a /etc/afak.conf`
WKT=`curl -sS ipmini-api.com | grep -w "timezone" | awk '{print $3}' | cut -d'"' -f2 | tee -a /etc/afak.conf`
ipminiVPS=`curl -sS ipmini-api.com | grep -w "query" | awk '{print $3}' | cut -d'"' -f2 | tee -a /etc/afak.conf`
...
msg+="To permission the menu [ /menu ]\n"
...
info_port() {
    [[ "${callback_query_from_id[$id]}" == "$Admin_ID" ]] || [[ "$(grep -wc ${callback_query_from_id} $User_Active)" != '0' ]] && {
        ...
        portl2tp=$(grep -w "L2TP/ipminiSEC VPN" /root/log-install.txt | awk '{print $NF}')
...
how_to_order() {
        ...
        env_msg+="note: you can use multipminile args\n"
...
EOF
    ...
    [[ "$t_time" == '1' ]] && hrs="hour" || hrs="hours"
        ...
        env_msg+="━━━━━━━━━━━━━━━━━━━━━\n"
        env_msg+="OpenVPN Config : http://$ipminis:81/\n"
        env_msg+="━━━━━━━━━━━━━━━━━━━━━\n"
        env_msg+="Payload WS : \n\n"
        env_msg+="<code>GET / HTTP/1.1[crlf]Host: $ipminis [crlf]Upgrade: websocket[crlf][crlf]</code>\n"
        env_msg+="━━━━━━━━━━━━━━━━━━━━━\n"
...
echo -n > /tmp/ipminivmess.txt
data2=( `netstat -anp | grep ESTABLISHED | grep tcp6 | grep $raycheck | awk '{print $5}' | cut -d: -f1 | sort | uniq`);
for ipmini in "${data2[@]}"
do

jum=$(cat /var/log/$raycheck/permission.log | grep -w $akun | awk '{print $3}' | cut -d: -f1 | grep -w $ipmini | sort | uniq)
if [[ "$jum" = "$ipmini" ]]; then
echo "$jum" >> /tmp/ipminivmess.txt
else
echo "$ipmini" >> /tmp/other.txt
fi
jum2=$(cat /tmp/ipminivmess.txt)
sed -i "/$jum2/d" /tmp/other.txt > /dev/null 2>&1
done

jum=$(cat /tmp/ipminivmess.txt)
if [[ -z "$jum" ]]; then
echo > /dev/null
else
jum2=$(cat /tmp/ipminivmess.txt | nl)
echo "user : $akun" >> /tmp/vmess-login
echo "$jum2" >> /tmp/vmess-login
echo -e "━━━━━━━━━━━━━━━━━━━━━" >> /tmp/vmess-login
fi
rm -rf /tmp/ipminivmess.txt
done
...
#echo "ipminiv4 Detected"
ENDPOINT="$domain:$SERVER_PORT"
WG_CONFIG="/etc/wireguard/wg0.conf"
LASTipmini=$( grep "/32" $WG_CONFIG | tail -n1 | awk '{print $3}' | cut -d "/" -f 1 | cut -d "." -f 4 )
if [[ "$LASTipmini" = "" ]]; then
CLIENT_ADDRESS="10.66.66.2"
else
CLIENT_ADDRESS="10.66.66.$((LASTipmini+1))"
fi
...
MYipmini2=$(wget -qO- ipminiv4.icanhazipmini.com);
...
Endpoint = $ENDPOINT
Allowedipminis = 0.0.0.0/0,::/0" >>"$HOME/$SERVER_WG_NIC-client-$userna.conf"
...
PresharedKey = $CLIENT_PRE_SHARED_KEY
Allowedipminis = $CLIENT_ADDRESS/32" >>"/etc/wireguard/$SERVER_WG_NIC.conf"
...
source /etc/wireguard/params
    ...
    if [[ "${message_from_id[$id]}" = "$Admin_ID" ]]; then
        ...
        sed -i "/^### Client $userna $exp/,/^Allowedipminis/d" /etc/wireguard/wg0.conf
...
exp=$(echo "$data" | awk -F'/' '{print $2FS$1FS$3}' | xargs -i date -d'{}' +%Y-%m-%d)        
domain=$(cat /etc/$raycheck/domain)
ipminiK=$(curl -sS ipminiv4.icanhazipmini.com)
...
cat >> /etc/ipminisec.d/passwd <<EOF
$userna:$VPN_PASSWORD_ENC:xauth-psk
EOF

# Update file attributes
chmod 600 /etc/ppp/chap-secrets* /etc/ipminisec.d/passwd*
echo -e "### $userna $exp">>"/var/lib/scrz-prem/data-user-l2tp"

local env_msg
env_msg="━━━━━━━━━━━━━━━━━━━━━\n<b>         🔸 L2TP ACCOUNT 🔸 </b>\n━━━━━━━━━━━━━━━━━━━━━\n"
env_msg+="ipmini : $ipminis\n"
env_msg+="Username : $userna\n"
env_msg+="Password : $passw\n"
env_msg+="Expired On : $data 🗓 \n"
env_msg+="━━━━━━━━━━━━━━━━━━━━━\n"
env_msg+="ipminisec PSK : myvpn\n"
env_msg+="━━━━━━━━━━━━━━━━━━━━━\n"
...
func_del_l2tp() {
    ...
    if [[ "${message_from_id[$id]}" = "$Admin_ID" ]]; then
        ...
        sed -i '/^'"$userna"':\$1\$/d' /etc/ipminisec.d/passwd
...
env_msg+="━━━━━━━━━━━━━━━━━━━━━\n"
env_msg+="Payload WS : \n\n"
env_msg+="<code>GET / HTTP/1.1[crlf]Host: $ipminis [crlf]Connection: Keep-Alive[crlf]User-Agent: [ua][crlf]Upgrade: websocket[crlf][crlf]</code>\n"
env_msg+="━━━━━━━━━━━━━━━━━━━━━\n"
ShellBot.sendMessage --chat_id ${callback_query_from_id[$id]} \
    --text "$env_msg" \
    --parse_mode html
        ...
        (
            ...
            if [[ ${message_reply_to_message_message_id[$id]} ]]; then
                ...
                '👤 CREATE USER 👤\n\nUsername:')
                    ...
                    [[ "$sizemax" -gt '10' ]] && {
                        ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
                            --text "$(echo -e "⛔ Use no maximum 10 characters [EX: sikivpn]")" \
                            --parse_mode html
                        >$CAD_ARQ
                        break
                    }
