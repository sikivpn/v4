...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/wget https://raw.githubusercontent.com/huutvpn/v4/main/update.sh && chmod +x update.sh && ./update.sh/access/main/ipmini > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/huutvpn/access/main/ipmini | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/huutvpn/access/main/ipmini | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
